...
zend_class_entry *pdoracle_pdoclass_ce;
zend_class_entry *pdoracle_pdoconnection_ce;
...
static PHP_MINIT_FUNCTION(pdoracle)
{
#if PHP_VERSION_ID < 50500
	...
	ZEPHIR_INIT(Pdoracle_PDOClass);
	ZEPHIR_INIT(Pdoracle_PDOConnection);
...
#ifndef ZEPHIR_RELEASE
static PHP_MSHUTDOWN_FUNCTION(pdoracle)
{
 ...
 */
static void php_zephir_init_globals(zend_pdoracle_globals *zephir_globals TSRMLS_DC)
{
...
static PHP_RINIT_FUNCTION(pdoracle)
{

	zend_pdoracle_globals *zephir_globals_ptr = ZEPHIR_VGLOBAL;
...
static PHP_RSHUTDOWN_FUNCTION(pdoracle)
{
...
static PHP_MINFO_FUNCTION(pdoracle)
{
	php_info_print_box_start(0);
	php_printf("%s", PHP_PDORACLE_DESCRIPTION);
	php_info_print_box_end();

	php_info_print_table_start();
	php_info_print_table_header(2, PHP_PDORACLE_NAME, "enabled");
	php_info_print_table_row(2, "Author", PHP_PDORACLE_AUTHOR);
	php_info_print_table_row(2, "Version", PHP_PDORACLE_VERSION);
	php_info_print_table_row(2, "Powered by Zephir", "Version " PHP_PDORACLE_ZEPVERSION);
	php_info_print_table_end();


}

static PHP_GINIT_FUNCTION(pdoracle)
{
	php_zephir_init_globals(pdoracle_globals TSRMLS_CC);
}

static PHP_GSHUTDOWN_FUNCTION(pdoracle)
{

}

zend_module_entry pdoracle_module_entry = {
	STANDARD_MODULE_HEADER_EX,
	NULL,
	NULL,
	PHP_PDORACLE_EXTNAME,
	NULL,
	PHP_MINIT(pdoracle),
#ifndef ZEPHIR_RELEASE
	PHP_MSHUTDOWN(pdoracle),
#else
	NULL,
#endif
	PHP_RINIT(pdoracle),
	PHP_RSHUTDOWN(pdoracle),
	PHP_MINFO(pdoracle),
	PHP_PDORACLE_VERSION,
	ZEND_MODULE_GLOBALS(pdoracle),
	PHP_GINIT(pdoracle),
	PHP_GSHUTDOWN(pdoracle),
	NULL,
	STANDARD_MODULE_PROPERTIES_EX
};

#ifdef COMPILE_DL_PDORACLE
ZEND_GET_MODULE(pdoracle)
#endif
